<template>
  <div>
    ...
    <div class="text-grey-6 text-body2 q-mb-md">
      Silver price updated at {{ formattedLastUpdate }}
    </div>
    ...
    <div class="row q-col-gutter-md q-mb-md">
      <div class="col-6">
        <q-card flat class="bg-white q-pa-md text-center rounded-small-borders">
          <div class="text-caption text-grey-6">Buy - Silver <br />(Fees Excl.)</div>
      ...
      <div class="col-6">
        <q-card flat class="bg-white q-pa-md text-center rounded-small-borders">
          ...
          <div class="text-h6 text-weight-bold q-mt-xs">
            {{ formatCurrency(silverRate.sellPrice) }}
          </div>
        </q-card>
      </div>
    </div>
    ...
    <div class="bg-white q-pa-md rounded-small-borders">
      ...
      <q-tab-panels v-model="tab" animated class="bg-transparent text-dark q-pa-none">
          ...
          <div v-if="selectedWeightId" class="q-mb-md">
            ...
            <div class="quantity-selector-rounded" :class="{ disabled: validSplits.length <= 1 }">
              <q-btn
                ...
                @click="prevSplit"
              />
              <!-- Show current split count (e.g. 1, 2, 4) -->
            ...
            >
              No valid combinations found in stock for this weight (Check your cart).
            </div>
          </div>
        ...
        <q-tab-panel name="smart" class="q-pa-none">
          ...
          <!-- SMART OPTIONS RESULT (Based on Input) -->
          <div v-if="generatedOptions.length > 0">
            ...
            >
              <div class="cart-content">
                <span class="text-weight-bold">Add Selected Package</span>
          ...
          <div v-else-if="targetWeight > 0" class="text-center q-pa-md text-grey">
            ...
            <div class="text-caption">
              Cannot build exactly {{ targetWeight }}g with current effective stock. <br />Check
              your cart or try standard weights.
            </div>
          </div>
        </q-tab-panel>
      </q-tab-panels>
    ...
    <!-- Shared: Weight Selection Dialog (For Manual Tab) -->
    <q-dialog v-model="showWeightDialog" position="bottom">
      <q-card class="weight-dialog">
        <q-card-section class="q-pb-none">
          ...
          <div class="text-caption text-grey-6">MFG: Manufacturing Fees</div>
        </q-card-section>

        <q-card-section class="scroll" style="max-height: 50vh">
          <div class="weight-options">
            <div
              v-for="product in silverInventory"
              :key="product.id"
            ...
            <div v-if="silverInventory.length === 0" class="text-center q-pa-lg text-grey">
    ...
    <q-dialog v-model="showSuccessDialog" persistent position="bottom">
      <q-card class="success-dialog">
        <q-card-section class="text-center q-pt-xl q-pb-md">
          ...
          <div class="text-body2 text-grey-6 q-mt-sm">Your items have been added successfully.</div>
        </q-card-section>
...
const silverInventory = computed(() => inventoryStore.availableSilverProducts || [])
const userBalance = computed(() => parseFloat(userStore.user?.wallet_balance || 0))

const formattedLastUpdate = computed(() => {
  if (!silverRate.value.updatedAt) return 'Loading...'
  return new Date(silverRate.value.updatedAt).toLocaleString()
})
...
const totalEffectiveWeight = computed(() => {
  return silverInventory.value.reduce((total, p) => {
...
// 1. Calculate Valid Splits for the selected weight
const validSplits = computed(() => {
  if (!selectedWeightId.value) return []

  const targetProduct = silverInventory.value.find((p) => p.id === selectedWeightId.value)
  ...
  silverInventory.value.forEach((p) => {
...
// 2. The currently active split configuration
...
// 3. Current Total Weight (should match selected dropdown weight)
...
const calculatedManualPrice = computed(() => {
  ...
  const basePrice = silverRate.value.buyPrice
  const weight = parseFloat(product.weight_grams)
  const fees = parseFloat(product.factoring_fees)

  // Formula: (Base Price * Weight) + Fees (per item) * Quantity
...
function calculateCombination(target, sortOrder = 'desc') {
  // Sort Inventory
  const sortedInventory = [...silverInventory.value].sort((a, b) => {
...
.add-cart-btn {
  height: 56px;
  background: #e0e0e0 !important; /* Silver metallic look */
...
.selected-card {
  border-color: #616161; /* Dark grey for silver selected */
